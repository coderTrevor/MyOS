...
FNT_xy FNT_Generate(const char* text, unsigned int len, unsigned int w, PIXEL_32BIT* pixels)
{
	unsigned int i, x, y, col, row, stop;
	unsigned char *fnt, chr;
    unsigned int rowSpace = 3; // put same space between rows to improve readability
	FNT_xy xy;
		...
		for(y = 0; y < FNT_FONTHEIGHT; y++)
        {
			for(x = 0; x < FNT_FONTWIDTH; x++)
            {
				if(fnt[text[i] * FNT_FONTHEIGHT + y] >> (7 - x) & 1)
					pixels[((col - 1) * FNT_FONTWIDTH) + x + (y + row * (FNT_FONTHEIGHT + rowSpace)) * w] = white;
                else
                    pixels[((col - 1) * FNT_FONTWIDTH) + x + (y + row * (FNT_FONTHEIGHT + rowSpace)) * w] = black;
			}
		}		
	}
...
{
	FNT_RenderMax(text, strlen(text));
}
...
void FNT_RenderMax(const char* text, unsigned int len)
{
	...
	FNT_Generate(text, len, graphicsWidth, (PIXEL_32BIT *)linearFrameBuffer);
